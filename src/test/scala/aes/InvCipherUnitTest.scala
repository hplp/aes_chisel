...
class InvCipherUnitTester(c: InvCipher, Nk: Int, InvSubBytes_SCD: Boolean) extends PeekPokeTester(c) {
  ...
  val state256 = Array(0x9a, 0x19, 0x88, 0x30, 0xff, 0x9a, 0x4e, 0x39, 0xec, 0x15, 0x01, 0x54, 0x7d, 0x4a, 0x6b, 0x1b)
  val state = Nk match {
    case 4 => state128
    case 6 => state192
    case 8 => state256
  ...
  val Key128 = Array(0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f)
  ...
  val Key192 = Array(0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17)
  ...
  val Key256 = Array(0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f)
  ...
  for (j <- 0 until Params.StateLength)
    poke(aes_icipher.io.roundKey(j), expandedKey(Nr)(j))
  // reset start
  poke(aes_icipher.io.start, 0)
  step(1)
  ...
  for (i <- 1 until Nrplus1) {
    for (j <- 0 until Params.StateLength)
      poke(aes_icipher.io.roundKey(j), expandedKey(Nr - i)(j))
    step(1)
  }
...
class InvCipherTester extends ChiselFlatSpec {
  ...
  private val InvSubBytes_SCD = true
  private val backendNames = Array("firrtl", "verilator")
  private val dir = "InvCipher"
  ...
  "Basic test using Driver.execute" should "be used as an alternative way to run specification" in {
    iotesters.Driver.execute(
      ...
      () => new InvCipher(Nk, InvSubBytes_SCD)) {
      c => new InvCipherUnitTester(c, Nk, InvSubBytes_SCD)
    } should be(true)
  }
    ...
    if (backendNames.contains("verilator")) {
      iotesters.Driver.execute(
        Array("--target-dir", "test_run_dir/" + dir + "_verilator_test", "--top-name", dir,
          "--backend-name", "verilator"), () => new InvCipher(Nk, InvSubBytes_SCD)) {
        c => new InvCipherUnitTester(c, Nk, InvSubBytes_SCD)
      } should be(true)
    }
  }
    ...
    if (backendNames.contains("firrtl")) {
      iotesters.Driver.execute(
        Array("--target-dir", "test_run_dir/" + dir + "_firrtl_test", "--top-name", dir,
          "--backend-name", "firrtl", "--generate-vcd-output", "on"), () => new InvCipher(Nk, InvSubBytes_SCD)) {
        c => new InvCipherUnitTester(c, Nk, InvSubBytes_SCD)
      } should be(true)
    }
  }
  ...
  "running with --is-verbose" should "show more about what's going on in your tester" in {
    iotesters.Driver.execute(
      Array("--target-dir", "test_run_dir/" + dir + "_verbose_test", "--top-name", dir,
        "--is-verbose"), () => new InvCipher(Nk, InvSubBytes_SCD)) {
      c => new InvCipherUnitTester(c, Nk, InvSubBytes_SCD)
    } should be(true)
  }
