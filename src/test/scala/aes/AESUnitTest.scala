...
class AESUnitTester(c: AES) extends PeekPokeTester(c) {

  private val aes = c
  var input_text = Array(0x32, 0x43, 0xf6, 0xa8, 0x88, 0x5a, 0x30, 0x8d, 0x31, 0x31, 0x98, 0xa2, 0xe0, 0x37, 0x07, 0x34)
...
class AESTester extends ChiselFlatSpec {
  ...
  for (backendName <- backendNames) {
    "AES" should s"execute cipher and inverse cipher (with ${backendName})" in {
      Driver(() => new AES, backendName) {
        c => new AESUnitTester(c)
      } should be(true)
    }
  }

  "running with --is-verbose" should "show more about what's going on in the tester" in {
    iotesters.Driver.execute(Array("--is-verbose"), () => new AES) {
      c => new AESUnitTester(c)
    } should be(true)
  }

  "running with --fint-write-vcd" should "create a vcd file from the test" in {
    iotesters.Driver.execute(Array("--fint-write-vcd"), () => new AES) {
      c => new AESUnitTester(c)
    } should be(true)
  }
}
