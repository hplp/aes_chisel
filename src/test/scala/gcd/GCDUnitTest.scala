...
/**
  ...
  * }}}
  * From a terminal shell, append '-- -z search_word' for a specific test:
  * {{{
  * sbt 'testOnly gcd.GCDTester -- -z verbose'
  * }}}
  */
class GCDTester extends ChiselFlatSpec {
  ...
  "Basic test using Driver.execute" should "be used as an alternative way to run specification" in {
    iotesters.Driver.execute(
      Array("--target-dir", "test_run_dir/" + dir + "_basic_test", "--top-name", dir), () => new GCD) {
      c => new GCDUnitTester(c)
    } should be(true)
  }
    ...
    if (backendNames.contains("verilator")) {
      iotesters.Driver.execute(
        Array("--target-dir", "test_run_dir/" + dir + "_verilator_test", "--top-name", dir,
          "--backend-name", "verilator"), () => new GCD) {
        c => new GCDUnitTester(c)
      } should be(true)
    }
  }
    ...
    if (backendNames.contains("firrtl")) {
      iotesters.Driver.execute(
        Array("--target-dir", "test_run_dir/" + dir + "_firrtl_test", "--top-name", dir,
          "--backend-name", "firrtl", "--generate-vcd-output", "on"), () => new GCD) {
        c => new GCDUnitTester(c)
      } should be(true)
    }
  }
